...
dhd_dbg_nan_event_handler(dhd_pub_t *dhdp, event_log_hdr_t *hdr, uint32 *data)
{
	...
	uint32 evt_payload_len = 0, tot_payload_len = 0;
	...
	nan_hdr.t = *data;
	DHD_RING(("%s: version %u event %x\n", __func__, nan_hdr.version, nan_hdr.event));
	...
	if (evt_match) {
		...
		for (i = 0; i < ARRAYSIZE(nan_evt_tag_map); i++) {
			tlv_data = (tlv_log *)event_get_tlv(nan_evt_tag_map[i].fw_id,
				tlvs, tlv_len);
			if (tlv_data) {
				DHD_RING(("NAN evt tlv.tag(%s), tlv.len : %d, tlv.data :  ",
	...
	void *raw_event_ptr, uint datalen)
{
	...
	bool event_type = FALSE, nan_event = FALSE;
	uint32 *log_ptr =  NULL;
	dll_t list_head, *cur;
			...
			if (!event_type) {
				...
				for (id = 0; id < ARRAYSIZE(fw_verbose_level_map); id++) {
					if (fw_verbose_level_map[id].tag == log_hdr->tag) {
						dhd_dbg_verboselog_handler(dhdp, log_hdr,
							raw_event_ptr);
					}
				}
			}
	...
	dbg_urgent_noti_t os_urgent_notifier, void *os_priv)
{
	...
	if (!dbg) {
		DHD_ERROR(("%s: Debug Ring allocation fail. No memory\n", __FUNCTION__));
		return BCME_NOMEM;
	}
	...
	if (ret) {
		DHD_ERROR(("%s: FW_VERBOSE_RING initialize fail.\n", __FUNCTION__));
		goto error;
	}
...
error:
	...
	DHD_ERROR(("%s: Debug Ring initalization fail. No memory\n", __FUNCTION__));
	return ret;
}
